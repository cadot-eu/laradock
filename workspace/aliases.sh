...
# Dans workspace/aliases.sh
sc() {
    local projet=$(echo "$PWD" | cut -d'/' -f4)
    php "/var/www/$projet/bin/console" "$@"
}

# Autocomplétion
_sc_autocomplete() {
    local projet=$(echo "$PWD" | cut -d'/' -f4)
    if [ -f "/var/www/$projet/bin/console" ]; then
        local commands=$(php "/var/www/$projet/bin/console" list --raw --no-ansi 2>/dev/null | awk '{print $1}')
        COMPREPLY=($(compgen -W "$commands" -- "${COMP_WORDS[1]}"))
    fi
}
complete -F _sc_autocomplete sc
